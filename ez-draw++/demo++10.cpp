...
private:
  void dessin_aiguille (unsigned long color, double angle, int xc, int yc, int d1, int d2, int ra)
  {
  ...
  void expose()
  {
    ...
    setFont(2);
    ostringstream oss;
    oss << setfill('0') << setw(2) << int(h) // heure sans la partie apres la virgule, sur une largeur de deux lettres completee au besoin avec des '0'.
        << ':'
        << setfill('0') << setw(2) << int(m);
    drawText(EZAlign::TC, getWidth()/2, 6, oss.str());

    dessin_aiguille (ez_red,  h*30, getWidth()/2, getHeight()/2, 20, 70, 10);
    dessin_aiguille (ez_blue, m* 6, getWidth()/2, getHeight()/2, 15, 92, 6);
